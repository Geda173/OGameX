...
@if (!empty($build_active))
        ...
        <tr class="data">
            <td colspan="2">
                ...
                <a class="build-faster dark_highlight tooltipLeft js_hideTipOnMobile building "
                   ...
                   onclick="halveBuildingTime({{ $dm_cost }}); return false;">
    ...
    <script type="text/javascript">
        ...
        var halveTimeUrl = @if(Request::is('resources*')) '{{ route('resources.halvetime') }}' @elseif(Request::is('facilities*')) '{{ route('facilities.halvetime') }}' @else '{{ route('resources.halvetime') }}' @endif;
        ...
        function halveBuildingTime(dmCost) {
            var question = 'Do you want to reduce the construction time by 50% for <span style="font-weight: bold;">' + dmCost.toLocaleString() + ' Dark Matter</span>?';
            errorBoxDecision('Halve Construction Time', question, 'Yes', 'No', function () {
                $.post(halveTimeUrl, {
                    _token: '{{ csrf_token() }}'
                }, function (data) {
                    if (data.success) {
