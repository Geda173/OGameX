...
class EspionageMission extends GameMission
{
    ...
    protected function processArrival(FleetMission $mission): void
    {
        ...
        // Notify the defender that a probe was detected on their planet.
        try {
            $er = \OGame\Models\EspionageReport::query()->find($reportId);
            if ($er) {
                // Store RAW coords (no BBCode) — we’ll format in the message class.
                $coordStr = $er->planet_galaxy . ':' . $er->planet_system . ':' . $er->planet_position;

                // Get attacker username (already in memory)
                $attackerUsername = $origin_planet->getPlayer()->getUsername();

                DB::table('messages')->insert([
                    'user_id'    => $er->planet_user_id,   // defender
                    'key'        => 'espionage_detected',
                    'subject'    => null,
                    'body'       => null,
                    'params'     => json_encode([
                        'attacker_username' => $attackerUsername,
                        'coords'            => $coordStr,   // RAW here
                        // 'planet_name'    => $target_planet->getName(), // optional
                        // 'chance'         => (int)$detectionChancePct,  // optional
            ...
            // Don’t break the mission if messaging fails; log it and continue.
            \Log::warning('Failed to insert espionage_detected message', ['mission_id' => $mission->id, 'error' => $e->getMessage()]);
        }
